...
export default function Participation() {
  ...
  // 2) When event changes: fetch coupons, reset date & stats
  useEffect(() => {
    setStats([]);                    // clear old stats immediately
  ...
  const fetchStats = useCallback(() => {
    if (!selectedEvent || couponNos.length===0 || range.length!==2) {
      setStats([]);  // ensure empty when no coupons
      return;
    }
    setLoading(true);
  ...
  // 4) Auto-fetch whenever event changes or date range changes
  useEffect(() => {
    fetchStats();
  }, [selectedEvent, range, fetchStats]);
